...
/*
 * ExtFastGPIO.h - changes for MEGA2560 pinouts - Brian Norman March 2017
 * 
 * Works ok with the exception of pins 42-49 PORT L and comms pins 14-15 PORT J and 16-17 PORT H
 * compiler throws an error "impossible constraint in 'asm'" highlighting
 * the line :-
 * 
 * _FG_SBI(pinStructs[pin].ddrAddr, pinStructs[pin].bit);
 * 
 * digitalWrite works on the above pins. I haven't figured out why yet but it is usable.
 * 
 * 
 */
...
#define IO_NONE 32
    ...
    const IOStruct pinStructs[] = {
        ...
        _FG_PIN(B, 1),
        _FG_PIN(B, 0),
        _FG_PIN(F, 0)  // Null pin (IO_NONE) analog 0
    };
